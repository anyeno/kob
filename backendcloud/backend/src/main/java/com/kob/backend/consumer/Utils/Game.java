...
public class Game extends Thread{
    ...
    private String status = "playing";   //playing 游戏中   finished  结束
    ...
    private boolean draw()  {
        ...
        for (int i = 0; i < this.inner_walls_count/2; i++) {
            for (int j = 0; j < 1000; j++) {
                int r = random.nextInt(this.rows);//返回0到rows-1之间的随机值
    ...
    private String getInput(Player player) {//将当前的局面信息编码成字符串
        //地图信息 # 自己的sx # 自己的sy # 自己的操作序列 # 对手的sx # 对手的sy # 对手的操作序列
        Player me, you;
    ...
    private void sendBotCode(Player player) {
        if(player.getBotId().equals(-1)) return; //真人亲自出马，不需要执行代码
    ...
    private boolean nextStep(){//等待两名玩家的下一步操作
        try {
            Thread.sleep(200);  //防止前端蛇还没移动完用户就输入了
